...
# Return an absolute path: absolute paths are returned as-is,
...
load_install_config() {
    ...
    local line key value owner_uid
...
check_tcp_port_free() {
    ...
    if command_exists ss; then
        if ss -ltn | awk -v p=":$port" 'NR>1 && index($4,p){found=1} END{exit found?0:1}'; then
            return 1
        fi
        return 0
    fi
...
# Randomize a config variable if it still holds its default value and
# the corresponding CLI override flag was not set.
# Usage: _randomize_if_default VAR_NAME DEFAULT FLAG_NAME EXCLUDE generator_cmd [args...]
...
# Write content produced by a generator function to a file with a controlled umask.
# Args: base_dir file_name generator_func [chmod_mode [label]]
...
EOF_COMPOSE
    ...
    # Top-level network declaration is always appended.
    cat <<'EOF_COMPOSE'
...
# Generator: lib-sui-control.sh
# NOTE: this is a single-quoted heredoc; \033 is a literal two-char sequence that
# printf '%b' will convert to an ESC byte at runtime in the generated file.
_gen_lib() {
    cat <<'EOF_LIB'
#!/usr/bin/env bash
set -euo pipefail
...
ACME_DIR="./acme"
SUI_PANEL_PORT=""
SUI_SUBSCRIPTION_PORT=""
SUI_PANEL_PATH=""
SUI_SUBSCRIPTION_PATH=""
COLOR_INFO='\033[0;32m'
COLOR_WARN='\033[1;33m'
COLOR_ERROR='\033[0;31m'
COLOR_RESET='\033[0m'
...
load_config_relative() {
    ...
    local owner_uid
    owner_uid="$(get_file_owner_uid "$config_file")"
    [[ -z "$owner_uid" || "$owner_uid" == "0" ]] || die "Config file must be owned by root: $config_file"
...
# Generator: s-ui-db-configure.sh
_gen_db() {
    cat <<'EOF_DB'
#!/usr/bin/env bash
set -euo pipefail
...
initialize_runtime_artifacts() {
    ...
    if [[ "$CERT_MODE" == "selfsigned" ]]; then
        ...
        # Restart so s-ui loads the freshly generated certificate.
        restart_sui_container "$INSTALL_DIR"
    elif [[ "$CERT_MODE" == "acme" ]]; then
        # Issue the initial certificate; acme-cert.sh restarts the container on success.
...
install_control_script() {
    ...
    # Randomize ports and paths that still hold their default values before
    # showing the interactive dialog so the user sees unique endpoints right away.
...
EOF_BANNER
    ...
    if [[ "$CERT_MODE" == "acme" ]]; then
        # Verify that Docker containers can reach the internet before attempting ACME issuance.
    ...
    # Copy the running script itself into the install directory.
    ...
    # Reload the config that was just written so all globals reflect the final install settings.
    ensure_config_loaded "$INSTALL_DIR"
    ...
    # Wait for s-ui to create and populate its database (up to 60 seconds).
...
# ----------------------------------------------------------------------
main() {
    # Resolve the absolute path of the running script.
    # /proc/$$/fd/255 and /dev/fd/255 are fallbacks for pipe-based execution.
    ...
    while [[ $# -gt 0 ]]; do
        case "$1" in
            ...
            --panel-port)
                require_option_value "$1" "${2-}"
                SUI_PANEL_PORT="$2"; CLI_PANEL_PORT_SET="1"; shift 2 ;;
            --subscription-port)
                require_option_value "$1" "${2-}"
                SUI_SUBSCRIPTION_PORT="$2"; CLI_SUBSCRIPTION_PORT_SET="1"; shift 2 ;;
            --panel-path)
                require_option_value "$1" "${2-}"
                SUI_PANEL_PATH="$2"; CLI_PANEL_PATH_SET="1"; shift 2 ;;
            --subscription-path)
                require_option_value "$1" "${2-}"
                SUI_SUBSCRIPTION_PATH="$2"; CLI_SUBSCRIPTION_PATH_SET="1"; shift 2 ;;
            --batch)
